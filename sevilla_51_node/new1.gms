...
* ============================================================
$include "C:\Users\freal\MATLAB\Projects\untitled\code\sevilla_net_49\new2.gms";
...
Binary variable capa(i,j);

Parameter logit_coef;
...
bud_def..
    bud =e= 1e-3*sum(i, station_capacity_slope(i)*sprim(i))
         + 1e-3*sum((i,j), link_capacity_slope(i,j)*aprim(i,j))
         + (1e-3*lam*sum((i,j),
...
op_def..
    op =e= 1e-3*sum((i,j), op_link_cost(i,j)*aprim(i,j));
...
pax_def..
    pax =e=
      1e-3*sum((o,d),
            demand(o,d)
          * sum((i,j), (logit_coef*prices(i,j))*fij(i,j,o,d)))
    + 1e-3*sum((o,d), demand(o,d)
          * (logit_coef*alt_price(o,d))*fext(o,d))

    + 1e-3*sum((o,d),demand(o,d)* ft(o,d))
*          * (  ( f(o,d)*log(f(o,d)) - f(o,d) ) ))

   + 1e-3*sum((o,d), demand(o,d)*ftext(o,d))
...
* Objetivo total
obj_def..
    obj =e= 1e-6*(alfa*pax+beta*bud+(1-alfa)*op);
...
cap_link(i,j)..
    sum((o,d), fij(i,j,o,d)*demand(o,d)) =l= tau*a(i,j)*a_nom;
...
def_deltasaux(i).. deltasaux(i) =e= congestion_coefs_stations(i)*deltas(i) + epsi;

def_deltaaaux(i,j).. deltaaaux(i,j) =e= congestion_coefs_links(i,j)*deltaa(i,j) + epsi;
